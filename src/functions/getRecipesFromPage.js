...
// pobiera wszystkie przepisy z jednej strony
const getRecipesFromPage = async (mainPage, recipeType) => {
  try {
    const aTags = $(
    ...
    for (let i = 0; i < aTags.length; i++) {
      ...
      // nazwa
      const recipeName = $('.step-container > meta[itemprop=name]', content)[
        '0'
      ].attribs.content
      ...
      try {
        ...
        for (let i = 0; i < nutrients.foods.length; i++) {
          ...
          values_per_100.protein =
            (product.values.protein * 100) / product.weight
          ...
          if (findedProduct) {
            ...
            if (
              ...
              calculatedIngredientValues.carbohydrates =
                Math.round(
                  findedProduct.values_per_100.carbohydrates * p * 100
                ) / 100
            } else {
      ...
      for (let i = 0; i < recipeIngredients.length; i++) {
        if (recipeIngredients[i].values) {
          ...
          recipeValues.carbohydrates +=
            recipeIngredients[i].values.carbohydrates
        }
      }
      ...
      //tworzenie przepisu
      const recipe = {
        name: recipeName[0].toUpperCase() + recipeName.slice(1),
        description:
...
module.exports = getRecipesFromPage
